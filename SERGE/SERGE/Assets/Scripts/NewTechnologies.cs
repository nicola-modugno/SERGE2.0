...
public class NewTechnologies : IEventCard
{
    string IEventCard.Name => "New Technologies or New Approaches";
    ...
    void IEventCard.modify(Dictionary<byte, List<RiskCard>> board)
    {
        ...
        for (byte i = 0; i < 3; i++)
        {
            for (byte j = 0; j < board[i].Count; j++)
            {
                ...
                {
                    Debug.LogError("Ho trovato ID-R1");
                ...
                {
                    Debug.LogError("Ho trovato ID-R5");
                    board[i][j].increaseImpatto();
                    board[i][j].increaseProbabilita();
                    secondCard = true;
